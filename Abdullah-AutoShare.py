...
\033[1;94m[+] TOLLS CRRATOR       :   MOHAMMED ABDULLAH 
\033[1;94m[+] FACEBOOK     :   Mohammed Abdullah 
\033[1;94m[+] GITHUB       :   AbdullahX4                 
\033[1;94m[+] TEAM         :   \33[1;42  MaX4 Squad\33[0m
\033[1;94m[+] TOOLS        :   FB AUTO SHARE TOOLS ðŸ’™ðŸ”¥
...
def login():
	...
	cetak(nel(f'\t\t{H2}FRESH COOKIE USE KORBEN \n\n \t\t  {N2} FRESH COOKIE ',subtitle=f'{P2}â”Œâ”€[ {K2}Cookies Room ]',subtitle_align= 'left',width=54, padding=1,style='blue'))
...
{B2} Current Date    :{H2} {hari}, {tanggal}''',title=f'{P2} {H2}[ {K2}Cookie Id Information {H2}]',subtitle_align='left',padding=1,style='blue'))
	...
	cetak(nel(f'{H2} H{H2}A{H2}S{H2}A{H2}N {B2}F{B2}B {O2}A{O2}U{O2}T{O2}O {N2}S{N2}H{N2}A{N2}R{N2}E {K2}S{K2}T{K2}A{K2}R{K2}T',subtitle=f'{P2}â”Œâ”€',subtitle_align='left',width=29,padding=0,style='blue'))
	...
	try:
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print(f'{P}\r   â””â”€â”€>{H} {bas} {K} H{K}A{K}S{K}A{K}N SEND {H}[{n}]{P}{K} AUTO SHARE {H} SUCSSES{H} ',end='');sys.stdout.flush()
			else:
				print("\n")
